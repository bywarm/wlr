...
def normalize_proxy_link(link: str) -> str:
    ...
    Дедупликация notworkers и сравнение выполняются по этим данным.
    """
    ...
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            if link and any(link.startswith(p) for p in _SUBSCRIPTION_PROTOCOLS):
                result.add(normalize_proxy_link(link))
    return result


def save_notworkers(path: str, links: set[str]) -> None:
    """Записывает множество ссылок в файл (один на строку), отсортированный для стабильного diff. Пустые строки не записываются."""
